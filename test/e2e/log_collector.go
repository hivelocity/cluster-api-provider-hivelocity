...
const (
	hivelocityPrivateKeyFilePath = "SSH_KEY_PATH"
)
...
func readPrivateKey() ([]byte, error) {
	privateKeyFilePath := os.Getenv(hivelocityPrivateKeyFilePath)
	if len(privateKeyFilePath) == 0 {
		return nil, fmt.Errorf("private key information missing. Please set %s environment variable", hivelocityPrivateKeyFilePath)
	}
