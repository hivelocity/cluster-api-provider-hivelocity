...
|
├── _artifacts            # This directory gets created during e2e tests. It contains yaml files and logs.
|
├── bin                   # Binaries. Mostly for envtests
|
├── config                # This is a Kubernetes manifest folder containing application resource configuration as
...
|
├── hack                  # This folder has scripts used for building, testing and developer workflow.
|
...
|
├── pkg                   # Go code which gets used to implement the controller.
├── templates
│   ├── cilium            # CNI work workload clusters (you can choose a different CNI, too)
